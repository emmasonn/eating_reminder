...
FirebaseSource({
  ...
  collection = firestore.FirebaseFirestore.instance.collection(collectionName);
}
...
.docs.map<T>((doc) => fromDocument(doc),).toList();
...
@override
Future<T?> viewItem(String id) async  => fromDocument((await collection.doc(id).get()));
...
@override
  Stream<List<T>> subscribeTo(List<WhereClause>? where) async* {
   ...
   if(where != null) {
      for(final whereClause in where) {
        if (whereClause.type == FilterType.equals) {
          query = query.where(whereClause.fieldName, isEqualTo: whereClause.value);
   ...
   yield* snapshots.map<List<T>>((firestore.QuerySnapshot snapshot) => snapshot.docs.map<T>(fromDocument).toList(),);
